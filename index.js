...
function processCard(cardData) {
    return new Promise((resolve, reject) => {
        ...
        try {
            ogs({ url }, (failed, { data, err }) => {
                if (failed) {
                	console.log('OGS had errors', err);
                    return reject(err);
                }
                if (!data) {
                ...
                console.log('OGS found some data');
...
getLists(process.env.BOARD)
    ...
    .then(cards => {
        console.log(`List has ${cards.length}`);
        return cards;
    })
    .then(cards => cards.filter(card => !card.idAttachmentCover && ( getUrl(card.name) || getUrl(card.desc) )))
    .then(cards => {
